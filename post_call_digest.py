...
CADENCE = {
    ...
    5: (16, "Clinical credibility",
        ...
        "\"We just had a baby and wanted to make sure our home was safe and free from harmful environmental toxins. "
        "The Lightwork assessment was incredibly thorough; they tested our air, water, lighting, EMF exposure, everything. "
        "They found issues we had no idea about and helped us fix them. The peace of mind that our home is safe and healthy "
        "for our family is invaluable. Highly recommend.\" "
        "- Dr. Rupy Aujla, Doctor & Author\n\n"
...
# SMTP (Gmail) for sending follow-up reminders to team members (local fallback)
SMTP_HOST = os.environ.get("SMTP_HOST", "smtp.gmail.com")
SMTP_PORT = int(os.environ.get("SMTP_PORT", "587"))
SMTP_EMAIL = os.environ.get("SMTP_EMAIL", "")
SMTP_PASSWORD = os.environ.get("SMTP_PASSWORD", "")

# Resend API (HTTPS-based email, used on Railway where SMTP is blocked)
RESEND_API_KEY = os.environ.get("RESEND_API_KEY", "")
RESEND_FROM = os.environ.get("RESEND_FROM", "Lightwork Digest <onboarding@resend.dev>")

# Gmail webhook (Google Apps Script) - sends via GmailApp.sendEmail() from your account
...
# SQLite draft cache
_DB_DIR = Path(os.environ.get("DB_DIR", str(SCRIPT_DIR)))
DRAFT_DB_PATH = _DB_DIR / "drafts.db"

# SQLite transcript cache (persistent across runs, avoids re-fetching from MCP)
TRANSCRIPT_DB_PATH = _DB_DIR / "transcripts.db"
...
def _send_owner_reminders(action_items_by_owner, date_str):
    ...
    """
    if not GMAIL_WEBHOOK_URL and not RESEND_API_KEY and (not SMTP_EMAIL or not SMTP_PASSWORD):
        print("\n  No email credentials set (GMAIL_WEBHOOK_URL, RESEND_API_KEY, or SMTP_EMAIL/SMTP_PASSWORD). Skipping email delivery.")
        return
    ...
    for owner, items in action_items_by_owner.items():
        ...
        # Send via Gmail webhook > Resend > SMTP (in priority order)
        sent = False

        if GMAIL_WEBHOOK_URL and not sent:
            try:
                payload = json.dumps({
                    "to": recipient,
                    "subject": subject,
                    "htmlBody": body_html,
                    "fromName": "Lightwork Follow-Up Tracker",
                    "secret": GMAIL_WEBHOOK_SECRET,
                }).encode()
                # Google Apps Script processes POST then 302-redirects to a
                # response URL. We POST via http.client (no auto-redirect),
                # then GET the redirect URL to read the JSON response.
                import http.client as _hc
                parsed = urllib.parse.urlparse(GMAIL_WEBHOOK_URL)
                conn = _hc.HTTPSConnection(parsed.hostname, timeout=30)
                ...
                if redirect_url:
                    ...
                    if result.get("status") == "ok":
                        print(f"  Reminder sent to {owner} ({recipient}) via Gmail webhook: {lead_count} lead{'s' if lead_count != 1 else ''}")
                        sent = True
                    else:
                        print(f"  Gmail webhook error for {owner}: {result.get('message', 'unknown error')}")
                else:
                    print(f"  Gmail webhook: unexpected response (no redirect) for {owner}")
            except Exception as e:
                print(f"  Gmail webhook failed for {owner} ({recipient}): {e}")

        if RESEND_API_KEY and not sent:
            try:
                payload = json.dumps({
                    "from": RESEND_FROM,
                    "to": [recipient],
                ...
                print(f"  Reminder sent to {owner} ({recipient}) via Resend: {lead_count} lead{'s' if lead_count != 1 else ''}")
                sent = True
            except Exception as e:
                print(f"  Error sending to {owner} ({recipient}) via Resend: {e}")

        if (SMTP_EMAIL and SMTP_PASSWORD) and not sent:
            ...
            try:
                ...
                print(f"  Reminder sent to {owner} ({recipient}) via SMTP: {lead_count} lead{'s' if lead_count != 1 else ''}")
                sent = True
            except Exception as e:
                print(f"  Error sending to {owner} ({recipient}) via SMTP: {e}")
