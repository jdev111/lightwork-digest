...
def _send_owner_reminders(action_items_by_owner, date_str):
    ...
    if not RESEND_API_KEY and (not SMTP_EMAIL or not SMTP_PASSWORD):
        print("\n  No email credentials set (RESEND_API_KEY or SMTP_EMAIL/SMTP_PASSWORD). Skipping email delivery.")
        return
    ...
    for owner, items in action_items_by_owner.items():
        ...
        # Send via Resend API (HTTPS) if available, otherwise fall back to SMTP
        if RESEND_API_KEY:
            try:
                payload = json.dumps({
                    "from": RESEND_FROM,
